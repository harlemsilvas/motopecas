      ...
      tr.innerHTML = `
        <td class="py-3">
          <div class="flex items-center gap-3">
            <img src="${imgSrc}" alt="${produto.nome}" class="w-12 h-12 object-cover rounded">
...
async function editarProduto(id) {
  try {
    ...
    window.renderProdutos(); // Força recarregar UI
    modo = "edicao";
  } catch (err) {
    ...
    btn.onclick = () => {
      ...
      input.onchange = (e) => {
        ...
        if (totalAtual + files.length > LIMITE_IMAGENS) {
          mostrarMensagem(
            `❌ Máximo de ${LIMITE_IMAGENS} imagens atingido.`,
            "red",
          );
          return;
        }
...
function atualizarListaImagensEdicao() {
  ...
  imagensEmEdicao.forEach((url, index) => {
    ...
    div.innerHTML = `
      ...
      <div class="absolute -bottom-1 -left-1 bg-blue-500 text-white text-xs w-5 h-5 rounded flex items-center justify-center cursor-move opacity-80 hover:opacity-100">≡</div>
    `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-page]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (page === "categorias") alert("Funcionalidade em desenvolvimento.");
      if (page === "itemDoDia") alert("Funcionalidade em desenvolvimento.");
    });
  });
