...
<head>
  ...
  <title>🔧 Painel Admin - MotoPeças</title>

  <!-- Carrega o CSS compilado do Tailwind -->
...
<body class="bg-gray-100 min-h-screen font-sans">
  <div class="flex h-screen">
    ...
    <aside class="w-64 bg-blue-800 text-white">
      <div class="p-6">
        <h1 class="text-xl font-bold">Admin MotoPeças</h1>
      </div>
      <nav class="mt-4">
        <a href="#" data-page="produtos" class="block py-3 px-6 bg-blue-900 font-medium">Produtos</a>
        <a href="#" data-page="categorias" class="block py-3 px-6 hover:bg-blue-700">Categorias</a>
        <a href="#" data-page="itemDoDia" class="block py-3 px-6 hover:bg-blue-700">Item do Dia</a>
      </nav>
    </aside>

    <!-- Conteúdo Principal -->
    <main id="adminContent" class="flex-1 p-6 overflow-y-auto bg-gray-50">
      <!-- Carregado dinamicamente pelos componentes -->
    </main>
  </div>

  <!-- Mensagem de feedback -->
  <div id="mensagem"
    class="hidden fixed top-4 right-4 z-50 px-4 py-2 rounded text-center font-medium shadow-lg max-w-xs"></div>
</body>
...
<form id="formCategoria" class="bg-white p-6 rounded-lg shadow mb-8">
  ...
  <button type="button" id="btnCancelarEdicaoCategoria"
    class="ml-3 bg-gray-400 text-white py-2 px-4 rounded hover:bg-gray-500 hidden">
    Cancelar
  </button>
</form>
...
tbody.innerHTML = `<tr>
  <td colspan="3" class="py-4 text-center text-gray-500">Nenhum item do dia cadastrado.</td>
</tr>`;
return;
}
...
if (isNaN(preco) || preco <= 0) { mostrarMensagem("❌ Preencha um preço válido.", "red" ); return; } if (
  precoPromocional && (isNaN(precoPromocional) || precoPromocional>= preco)
  ) {
  mostrarMensagem(
  ...
  */
  ...
  <div
    class="absolute -bottom-1 -left-1 bg-blue-500 text-white text-xs w-5 h-5 rounded flex items-center justify-center cursor-move opacity-80 hover:opacity-100"
  ...
  <div
    class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white text-xs font-bold rounded">
    NOVA</div>
  ...
  if (offset < 0 && offset> closest.offset) {
    ...
    * Carregar categorias no <select>
      ...
      <form id="formProduto" class="bg-white shadow-lg rounded-lg p-6 mb-8">
        ...
        <div class="mb-6">
          ...
          <select id="selectCategorias" multiple
            class="w-full border border-gray-300 rounded-lg px-3 py-2 h-24 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></select>
        </div>
        <div id="containerImagensEdicao" class="hidden mb-6">
          ...
          <button type="button" id="btnAdicionarImagem" class="mt-2 text-sm text-blue-600 hover:underline">+ Adicionar
            mais imagens</button>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 pt-4">
          <button type="submit" id="btnSubmit"
            class="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-lg shadow transition transform hover:scale-105 flex items-center justify-center gap-2">
          ...
          <button type="button" id="btnCancelarEdicao"
            class="hidden sm:flex items-center justify-center gap-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-3 px-6 rounded-lg transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
      ...
      */
      ...
      mensagem.className = `fixed top-4 right-4 z-50 p-4 rounded bg-${cor}-100 text-${cor}-800 font-medium shadow-lg
      transition-opacity duration-300 max-w-sm`;
